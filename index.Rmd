...
library(jpeg)
library(biOps)
image <- readJpeg("assets/fig/DB1j.jpg")
plot(image)
...
library(jpeg)
library(biOps)
image <- readJpeg("assets/fig/IB1j.jpg")
plot(image)
...
```{r}
d <- Sys.Date()
t <- paste(weekdays(d),months(d),as.numeric(format(d,"%d")),as.numeric(format(d,"%Y")))

```


### Today is `r sprintf(t)`, start planning for your retirement now.
